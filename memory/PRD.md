...
## Changelog
- 2026-02-12: **Product detail page cleaner rules.** Spec table auto-hides rows whose value is empty / `-` / `—` / `N/A` / `NA` / `not available` / `unknown` / `none` / `null` (case-insensitive). "Currently unavailable" replaced everywhere with "**Available on request**" (top stock line + side-panel). `formatProductPrice` now returns "Price on request" whenever `price` is missing, `null`, or `≤ 0` — regardless of `price_display`. Rating row hidden when 0. Every specs tab now shows a persistent gold-bordered note: "**Made-to-order — Specifications can be customised as per requirement. Please inquire for exact size, holder type, finish, and pricing.**"
