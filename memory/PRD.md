...
## Changelog
- 2026-02-12: **Catalogue PDF fully rebuilt.** Was: html2pdf snapshotting a scrollable web page → cut cards, black image boxes, huge blank areas, 81 broken pages. Now: proper A4-per-`<Page>` architecture. 27 clean pages total (cover · TOC · About · Reasons · 5 category dividers · 16 product pages @ 2-per-page · contact). All product images pre-converted to data-URLs via `/api/proxy-image` before snapshotting → zero CORS issues, zero black boxes; missing images show a "Image coming soon" branded SVG placeholder. `break-inside: avoid` on `.pdf-card` + `.pdf-toc-row` — no cut cards. Each `.pdf-page` fixed at `210mm × 296mm` so `html2canvas` produces exactly one PDF page per section. Every page has a gold hairline footer with brand · section · Page X / 27. Titles no longer use `background-clip: text` (unsupported by html2canvas) so they render as visible gold serif instead of empty rectangles. Verified against actual 8.5MB PDF output: correct page count, correct titles, correct images, no cut cards.
