...
## Changelog
- 2026-02-12: **"Send catalogue via WhatsApp" lead capture on Contact page.** New endpoint `POST /api/catalogue-request` stores every request in the `inquiries` collection with `type: "catalogue_request"` (visible in Admin → Inquiries alongside product enquiries). New `CatalogueOnWhatsApp` component renders a compact card under the Contact-page info column: name + `+91`-prefixed phone → server-side lead save → success state offers two CTAs (Download PDF now · Open in WhatsApp with a pre-filled message to the business number containing the visitor's name, phone, and a direct link to `/catalogue?print=1`). Phone parser accepts 10-digit, +91-prefixed, and 0-prefixed formats.
