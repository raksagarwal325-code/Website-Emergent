...
## Changelog
- 2026-02-13: **"Next available SKU" hint on New Product form.** Below the SKU field, a small `NEXT AVAILABLE: SGE-CH-027` link now appears the moment a category is selected. Prefix is derived from a 9-entry `CATEGORY_PREFIX` map (Chandelier→CH, Hanging Light→HL, Table Lamp→TL, Wall Light→WL, Candle Stand→CS, Floor Lamp→FL, Sconce→SC, Pendant Light→PL, Custom Design→CD) with a first-two-letter fallback for any custom category. Number is `max(existing numeric-suffixed SKUs for that prefix) + 1`, zero-padded to 3 digits. Clicking the hint (`p-sku-suggest`) fills the SKU field instantly. Verified live: Chandelier → SGE-CH-027, Table Lamp → SGE-TL-009, Sconce (empty category) → SGE-SC-001, click-to-fill works.
