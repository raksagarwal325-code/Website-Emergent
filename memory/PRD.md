...
## Changelog
- 2026-02-11: **Auto-pull cover — upgraded to headless Chromium (Playwright).** Instagram no longer exposes `og:image` to plain HTTP scrapers, so the endpoint now spins up a real headless browser server-side, waits for the SPA to render, and extracts the poster image via the `<img alt="Video by …">` / `<img alt="Photo by …">` accessibility label (very stable pattern). Pulled cover is downloaded and cached in Emergent object storage; Instagram is never called again for that reel. Verified live on the Samrat mysa_space reel — real reel poster now shows on the "As Styled By" card. ~6s per pull (browser cold start). Added `playwright` to requirements + `PLAYWRIGHT_BROWSERS_PATH=/pw-browsers` to backend `.env`.
