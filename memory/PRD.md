...
## Changelog
- 2026-02-11: **"As Styled By" entrance animations (Framer Motion).** Header and CTA fade+slide in when the section enters the viewport; cards reveal in sequence with a 120ms stagger (7 × 40px slide-up, editorial ease-out cubic-bezier). Cards get a 6px hover-lift. `once: true` so animations only fire on first scroll into view — no re-flicker on scroll back.
