...
## Changelog
- 2026-02-11: **"As Styled By" is now a carousel + full `/styled-by` gallery page.** Homepage carousel: 3 cards on desktop / 2 on tablet / 1 on mobile via `useVisibleCount` matchMedia hook. Auto-slides every 5s (pauses on hover + when tab hidden), continuous loop, prev/next arrows (edge-anchored on desktop, stacked row on mobile), gold pill-expanding dot pagination. Arrows/dots hide when items ≤ visible count. New gold "View All Styled Looks" button routes to `/styled-by` where the entire library renders in the full-width `flex-wrap justify-center` gallery grid (with creator counter, back-to-home link, and Instagram profile CTA). Card component extracted to `components/InfluencerCard.jsx` so carousel + gallery stay visually identical.
