...
## Changelog
- 2026-02-12: **AI Product Details Generator (Gemini 3 Flash + vision).** New `POST /api/ai/generate-product` and `POST /api/ai/generate-products-bulk` endpoints. Admin uploads one or many product photos; each is analysed and turned into a draft product with `status: "draft"`, badge `"Needs Review"`, `price_display: "on_request"`, price 0 — never auto-published. Draft includes: premium name, SEO name (60-70 chars), category (validated against allowed list), SKU (`SGE-<CAT>-<XYZ>` pattern), 1-line short description, 3-5 para full description in Indian-luxury tone, 8-14 comma-separated tags, and 11-key specs dict (Material, Finish, Glass Type, Product Type, Holder Type, Suitable For, Style, Color, Package Includes, Care Instructions, Customization Available). The prompt explicitly forbids inventing measurements/wattage/holder/material — uncertain fields are hedged ("appears to feature", "crystal-look") or left blank for admin review. New `AIProductGenerator` component adds a dropzone card at the top of Admin → Products with per-image status pills, review buttons that open the edit form, and safety caps. Product list now shows a gold "Needs Review" badge on draft rows and a status radio (Published / Draft) in the edit form. Public `/api/products` filters out drafts; admin passes `include_drafts=1`. Emergent LLM Key used — ~10-15s per image on Gemini 3 Flash.
