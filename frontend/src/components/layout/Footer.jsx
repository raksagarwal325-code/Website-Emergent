  ...
  const inner = (
    <div className="flex items-start gap-3 group">
      ...
      <div className="min-w-0">
        ...
        <div className="text-sm text-white/70 group-hover:text-white transition-colors leading-snug break-words">{content}</div>
      </div>
    </div>
  );
...
const EXPLORE_LINKS = [
  ...
  { label: "Download Catalogue PDF", href: "/catalogue?print=1", external: true, icon: Download },
];
...
export default function Footer() {
  ...
  const SUPPORT_LINKS = [
    ...
    { label: "Return & Replacement", href: "/legal/returns" },
    { label: "Payment Policy", href: "/legal/payment" },
      ...
      <div className="relative max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12">
          ...
          <div className="lg:col-span-4">
            ...
            {socials.length > 0 && (
              <div className="mt-7">
                <div className="text-[10px] uppercase tracking-[0.28em] text-white/40 mb-3">Follow</div>
          ...
          {/* Contact */}
          <div className="lg:col-span-2" data-testid="footer-contact-col">
            ...
            <div className="space-y-3.5">
              ...
              {emailHref && (
                <ContactRow icon={Mail} label="Email" value={settings?.admin_email} href={emailHref} testId="footer-contact-email" />
              )}
              {settings?.address && (
                <ContactRow icon={MapPin} label="Showroom" testId="footer-contact-address">
                  <span className="whitespace-pre-line">{settings.address}</span>
                  {mapsHref && (
                    <a href={mapsHref} target="_blank" rel="noreferrer" data-testid="footer-contact-maps" className="mt-1.5 inline-flex items-center gap-1 text-[10px] uppercase tracking-[0.24em] text-[#D4AF37] hover:text-[#B5952F]">
        ...
        {/* Bottom row */}
        <div className="mt-14 pt-6 border-t border-[#BF9972]/15 flex flex-col md:flex-row items-center justify-between gap-3 text-[11px] text-white/40">
          <div data-testid="footer-copyright">
            © {new Date().getFullYear()} {settings?.brand_name || "Samrat Glass Emporium"}. All rights reserved.
          </div>
          <div className="tracking-[0.24em] uppercase text-[10px]">Handcrafted in Firozabad</div>
        </div>
      </div>
    </footer>
  );
