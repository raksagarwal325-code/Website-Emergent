...
export default function FounderTeaser() {
  ...
  const portraitScale = useTransform(scrollYProgress, [0, 0.5, 1], [0.95, 1.02, 1]);
  const portraitOpacity = useTransform(scrollYProgress, [0, 0.18, 1], [0.65, 1, 1]);
  const copyY = useTransform(scrollYProgress, [0, 0.5, 1], [42, 0, -34]);
  const copyOpacity = useTransform(scrollYProgress, [0, 0.16, 0.86, 1], [0.35, 1, 1, 0.55]);
  const yearY = useTransform(scrollYProgress, [0, 1], [30, -35]);
  ...
  return (
    <section ref={sectionRef} data-testid="founder-teaser" className="relative isolate h-auto md:h-[145vh] px-6 bg-[#16070f]">
      <div className="max-w-7xl mx-auto md:sticky md:top-20 md:h-[calc(100vh-5rem)] flex items-center py-20 md:py-8 overflow-hidden">
        <div className="relative warm-panel w-full overflow-hidden min-h-[620px] md:min-h-0 md:h-[72vh] flex items-center">
          ...
          <div className="relative z-10 grid grid-cols-1 md:grid-cols-[0.85fr_1.15fr] gap-10 md:gap-14 items-center w-full p-8 md:p-12 lg:p-14">
            <motion.div className="relative flex justify-center" style={{ scale: prefersReducedMotion ? 1 : portraitScale, opacity: prefersReducedMotion ? 1 : portraitOpacity }}>
              <div className="relative">
                <div data-testid="founder-teaser-photo" className="overflow-hidden brand-glow border border-[#D4AF37]/35 w-[260px] h-[340px] md:w-[290px] md:h-[390px]">
            ...
            <motion.div className="text-center md:text-left will-change-transform min-w-0" style={{ y: prefersReducedMotion ? 0 : copyY, opacity: prefersReducedMotion ? 1 : copyOpacity }}>
              ...
              <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl leading-[1.04] mb-5 max-w-3xl text-balance">{title}</h2>
              <motion.div aria-hidden className="h-px w-44 bg-gradient-to-r from-[#D4AF37] to-transparent origin-left mb-6 mx-auto md:mx-0" style={{ scaleX: prefersReducedMotion ? 1 : ruleScale }} />
              <p className="text-white/70 leading-relaxed md:text-base lg:text-lg max-w-2xl">{body}</p>
              {f.name && <div className="mt-6 flex flex-wrap items-center gap-3 justify-center md:justify-start text-[11px] uppercase tracking-[0.28em] text-[#BF9972]"><span className="inline-block w-8 h-px bg-[#D4AF37]/60" />{f.name}{f.signature && <><span className="text-white/40">·</span><span className="text-white/50 tracking-normal normal-case text-sm italic">Since 1981</span></>}</div>}
              <div className="mt-7 flex justify-center md:justify-start"><Link to={cta_link} data-testid="founder-teaser-cta" className="group inline-flex items-center gap-2 border border-[#D4AF37]/60 hover:border-[#D4AF37] hover:bg-[#D4AF37] hover:text-black transition-colors px-7 py-3.5 uppercase text-xs tracking-[0.28em] text-[#D4AF37]">{cta_text}<ArrowUpRight size={14} className="transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></Link></div>
            </motion.div>
          </div>
        </div>
      </div>
