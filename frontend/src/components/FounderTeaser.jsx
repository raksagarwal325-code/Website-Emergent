...
export default function FounderTeaser() {
  ...
  const portraitScale = useTransform(scrollYProgress, [0, 0.45, 1], [0.88, 1.04, 1]);
  const portraitOpacity = useTransform(scrollYProgress, [0, 0.18, 1], [0.35, 1, 1]);
  const copyY = useTransform(scrollYProgress, [0, 0.42, 0.78, 1], [140, 0, 0, -110]);
  const copyOpacity = useTransform(scrollYProgress, [0, 0.2, 0.78, 1], [0, 1, 1, 0.2]);
  const yearY = useTransform(scrollYProgress, [0, 1], [80, -110]);
  ...
  return (
    <section ref={sectionRef} data-testid="founder-teaser" className="relative h-auto md:h-[190vh] px-6">
      <div className="max-w-7xl mx-auto md:sticky md:top-0 md:h-screen flex items-center py-20 md:py-0 overflow-hidden">
        <div className="relative warm-panel w-full overflow-hidden min-h-[620px] md:min-h-0 md:h-[78vh] flex items-center">
          ...
          <div className="relative z-10 grid grid-cols-1 md:grid-cols-[0.85fr_1.15fr] gap-10 md:gap-16 items-center w-full p-8 md:p-14 lg:p-16">
            <motion.div className="relative flex justify-center" style={{ scale: prefersReducedMotion ? 1 : portraitScale, opacity: prefersReducedMotion ? 1 : portraitOpacity }}>
              <div className="relative">
                <div data-testid="founder-teaser-photo" className="overflow-hidden brand-glow border border-[#D4AF37]/35 w-[260px] h-[340px] md:w-[310px] md:h-[430px]">
            ...
            <motion.div className="text-center md:text-left will-change-transform" style={{ y: prefersReducedMotion ? 0 : copyY, opacity: prefersReducedMotion ? 1 : copyOpacity }}>
              <div className="eyebrow mb-4 text-[#D4AF37]">{eyebrow}</div>
              <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl xl:text-7xl leading-[1.02] mb-6 max-w-3xl">{title}</h2>
              ...
              <p className="text-white/70 leading-relaxed md:text-lg max-w-2xl">{body}</p>
              {f.name && <div className="mt-7 flex items-center gap-3 justify-center md:justify-start text-[11px] uppercase tracking-[0.32em] text-[#BF9972]"><span className="inline-block w-8 h-px bg-[#D4AF37]/60" />{f.name}{f.signature && <><span className="text-white/40">·</span><span className="text-white/50 tracking-normal normal-case text-sm italic">Since 1981</span></>}</div>}
