    ...
    <section ref={sectionRef} data-testid="atelier-section" className="relative isolate overflow-hidden border-y border-white/10 bg-[#16070f] px-6 py-20 md:py-28">
      ...
      <div className="relative mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 md:grid-cols-12 md:gap-14">
        ...
        >
          ...
          {slides.length > 1 && <div className="mt-4 flex flex-wrap items-center justify-center gap-1.5" data-testid="atelier-dots">{slides.map((_, i) => <button key={i} data-testid={`atelier-dot-${i}`} aria-label={`View slide ${i + 1}`} aria-current={i === active ? "true" : undefined} onClick={() => setActive(i)} className="flex min-h-[44px] min-w-[44px] items-center justify-center focus:outline-none focus-visible:ring-1 focus-visible:ring-[#D4AF37]"><span aria-hidden="true" className={`h-2 rounded-full transition-all ${i === active ? "w-9 bg-[#D4AF37]" : "w-4 bg-white/25 hover:bg-white/45"}`} /></button>)}</div>}
        </motion.div>
