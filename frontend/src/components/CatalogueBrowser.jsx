  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      api.listProducts(buildParams(currentPage)).then((res) => {
        ...
        setTotal(nextTotal);
        setTotalPages(tp);
        setResolvedQuery(res?.resolved_query || "");
  ...
  return (
    <>
      <div className="mb-7 flex flex-col gap-4 no-print md:flex-row md:items-center">
        ...
        <div className="w-full md:w-52">
          <Select value={sort} onValueChange={updateSort}>
            <SelectTrigger aria-label="Sort catalogue" data-testid="sort-select" className="h-12 rounded-none border-white/15 bg-[#0a0a0a] text-white"><SelectValue placeholder="Sort" /></SelectTrigger>
      ...
      <div ref={gridTopRef} className="scroll-mt-56">
        {suggestion && !loading && (
          <div data-testid="catalog-search-suggestion" className="mb-5 border border-[#D4AF37]/25 bg-[#D4AF37]/[0.06] px-4 py-3 text-sm text-white/70">
            Showing results for <span className="font-medium text-[#D4AF37]">“{resolvedQuery}”</span>. Your search was “{q}”.
          </div>
        )}
