...
export default function AIProductGenerator({ products = [], onDone, setEditingProduct }) {
  ...
  return <section className="border border-[#D4AF37]/35 bg-[#0d0510] p-5 md:p-6 space-y-5" data-testid="ai-product-generator">
    ...
    {rows.length > 0 && <div className="overflow-x-auto border border-white/10"><table className="w-full min-w-[1050px] text-sm"><thead className="bg-black/40 text-[10px] uppercase tracking-widest text-white/45"><tr>{["Use", "Images", "Category", "Height", "Width", "Family / reference / facts", "Result", ""].map((h) => <th key={h} className="p-3 text-left">{h}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.client_id} className="border-t border-white/10 align-top">
      ...
      <td className="p-3"><select value={row.category} disabled={busy || !!row.draft} onChange={(e) => update(row.client_id, "category", e.target.value)} className="bg-black border border-white/15 px-2 py-2"><option value="">Auto-detect category</option>{CATEGORIES.map((c) => <option key={c}>{c}</option>)}</select></td>
      ...
      <td className="p-3"><textarea value={row.notes} disabled={busy || !!row.draft} onChange={(e) => update(row.client_id, "notes", e.target.value)} placeholder="Type only confirmed family, SKU references, counts or other facts…" rows={2} className="w-72 bg-black border border-white/15 px-2 py-2" /><div className="mt-1 text-[9px] leading-4 text-white/35">Optional. Example: Fanoos family; matches FL-13 and 16. The catalogue scan runs even when this is empty.</div>
      ...
      <td className="p-3 max-w-xs"><div className="text-[10px] uppercase tracking-widest"><Status row={row} /></div>{row.draft && <div className="mt-3 border-l-2 border-[#D4AF37] pl-3 py-1"><div className="text-[9px] uppercase tracking-[0.2em] text-[#D4AF37]">Suggested product name</div><div data-testid={`suggested-product-name-${row.client_id}`} className="font-serif text-lg leading-snug mt-1 text-white">{row.draft.name}</div><div className="text-[10px] text-[#BF9972] mt-1">{row.draft.sku} · {Object.keys(row.draft.specs || {}).length} specifications</div>{row.draft.sop_evidence?.matched_references?.length > 0 && <div className="mt-2 text-[9px] leading-4 text-white/45">Exact evidence: {row.draft.sop_evidence.matched_references.map((item) => `${item.sku} — ${item.name}`).join("; ")}</div>}{row.draft.sop_evidence?.automatic_catalogue_matches?.length > 0 && <div className="mt-2 space-y-1 text-[9px] leading-4 text-white/50"><div className="uppercase tracking-wider text-[#D4AF37]">Catalogue scan</div>{row.draft.sop_evidence.automatic_catalogue_matches.map((item) => <div key={item.sku} className="flex items-center gap-2 border border-white/10 p-1">{item.image && <img src={item.image} alt="" className="h-8 w-8 shrink-0 bg-black object-contain" />}<div className="min-w-0"><div className="truncate text-white/70">{item.sku} — {item.name}</div><div>{item.relation.replaceAll("_", " ")} · {Math.round(item.confidence * 100)}%</div></div></div>)}</div>}</div>}{[...(row.warnings || []), ...(row.validation || [])].map((w) => <div key={w} className="text-[10px] text-amber-300 mt-1">⚠ {w}</div>)}{row.error && <div className="text-[10px] text-red-400 mt-1">{row.error}</div>}{row.draft && row.state === "ready" && <ProductDraftConversation compact product={row.draft} imageFilenames={row.files.map((file) => file.name)} onApply={(draft, meta) => patchRow(row.client_id, { draft, validation: meta.validation, warnings: meta.warnings, state: row.state === "created" ? "ready" : row.state })} />}{row.state === "created" && <button onClick={() => setEditingProduct?.(row.draft)} className="mt-2 text-[10px] uppercase tracking-widest text-[#D4AF37]"><Edit3 size={10} className="inline mr-1" />Review & edit</button>}</td>
