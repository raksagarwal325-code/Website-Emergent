...
export default function ReasonsSection({ heading = "Reasons Why We Are Better", eyebrow = "Why choose us", compact = false }) {
  return (
    <section data-testid="reasons-section" className="relative overflow-hidden">
      ...
      <div className={`relative max-w-7xl mx-auto px-6 ${compact ? "py-16" : "py-24"}`}>
        <div className="text-center mb-12 max-w-2xl mx-auto">
          ...
          <h2 className="font-serif text-3xl sm:text-4xl lg:text-5xl leading-tight brand-gradient-text">{heading}</h2>
          <div className="mt-4 mx-auto h-px w-16 bg-[#D4AF37]/60"></div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {REASONS.map((r, i) => (
            <div key={r.title} data-testid={`reason-${i}`} className="warm-panel p-6 group hover:border-[#D4AF37]/50 transition-colors">
              <div className="w-10 h-10 mb-4 flex items-center justify-center border border-[#D4AF37]/40 text-[#D4AF37] group-hover:bg-[#D4AF37]/10 transition-colors">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d={ICONS[i]}/></svg>
              </div>
              <div className="font-serif text-base md:text-lg leading-snug text-white mb-2">{r.title}</div>
              <p className="text-white/60 text-xs md:text-sm leading-relaxed">{r.body}</p>
            </div>
          ))}
        </div>
      </div>
