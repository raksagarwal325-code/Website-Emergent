...
export default function AdminAuthGate({ children }) {
  ...
  if (state === "checking") {
    return <div className="min-h-screen bg-[#0a0a0a] text-white/60 flex items-center justify-center text-sm tracking-widest uppercase">Verifying session…</div>;
  }
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4">
        <div className="max-w-sm w-full border border-red-500/40 p-8 text-center space-y-4">
          ...
          <button onClick={signIn} className="mt-2 inline-flex items-center gap-2 text-[11px] uppercase tracking-widest border border-[#D4AF37]/60 text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black transition px-4 py-2" data-testid="admin-signin-btn"><LogIn size={14} /> Try a different account</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4">
        <div className="max-w-sm w-full border border-white/10 p-8 text-center space-y-4">
          ...
          <p className="text-white/50 text-xs leading-relaxed">Only allowlisted Google accounts can access the Samrat Glass Emporium admin panel.</p>
          {error && <p className="text-red-400 text-xs">{error}</p>}
          <button onClick={signIn} data-testid="admin-signin-btn" className="w-full inline-flex items-center justify-center gap-2 text-xs uppercase tracking-widest border border-[#D4AF37]/60 text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black transition px-4 py-3"><LogIn size={14} /> Sign in with Google</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="border-b border-white/10 bg-[#0a0a0a] px-6 py-2 flex flex-wrap items-center justify-between gap-3 text-xs text-white/60">
