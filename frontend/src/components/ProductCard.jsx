    ...
    >
      {badge && (
        <div
          ...
          className="absolute top-2 left-2 sm:top-4 sm:left-4 z-10 inline-flex items-center gap-1 border border-[#BF9972]/50 bg-black/60 backdrop-blur-sm px-2 py-1 text-[8px] sm:text-[10px] uppercase tracking-[0.18em] sm:tracking-[0.22em] text-[#D4AF37]"
        >
      ...
      <button
        ...
        className={`absolute top-2 right-2 sm:top-4 sm:right-4 z-10 h-9 w-9 sm:h-10 sm:w-10 rounded-full flex items-center justify-center border backdrop-blur-md transition-all ${
          fav
        ...
        >
          {img ? (
            <img
              src={img}
              alt={product.name}
              className="product-image block h-full w-full object-contain object-center p-1.5 opacity-95 group-hover:opacity-100 sm:p-3"
              loading="lazy"
          ...
          {projectCount > 0 && (
            <div
              ...
              className="absolute bottom-2 left-2 sm:bottom-3 sm:left-3 inline-flex items-center gap-1 border border-[#D4AF37]/40 bg-black/70 backdrop-blur-sm px-2 py-1 text-[8px] sm:text-[10px] uppercase tracking-[0.16em] sm:tracking-[0.2em] text-[#D4AF37]"
              title={`Featured in ${projectCount} real installation${projectCount === 1 ? "" : "s"}`}
            >
              <Sparkles size={9} strokeWidth={1.6} />
              <span className="hidden sm:inline">Featured in </span>{projectCount} project{projectCount === 1 ? "" : "s"}
            </div>
          )}
        </div>
      </Link>
      ...
      <div className="flex flex-col flex-1 p-3 sm:p-5">
        ...
        <Link to={productPath(product)} className="font-serif text-[15px] sm:text-lg leading-snug text-white hover:text-[#D4AF37] transition-colors min-h-[3.9rem] sm:min-h-[3.5rem] break-words line-clamp-3 sm:line-clamp-none">
          {product.name}
        </Link>
            ...
            {(() => {
              ...
              if (p.onRequest) {
                return (
                  <span
                    ...
                    className="text-[#D4AF37] font-serif text-[13px] sm:text-base font-medium truncate"
                  >
                    Price on request
                  </span>
                );
              ...
              return (
                <>
                  <span
                    data-testid={`product-price-${product.id}`}
                    className="text-[#D4AF37] font-serif text-[15px] sm:text-lg truncate"
                  >
                    {p.label && (
                      <span className="text-[8px] sm:text-[10px] uppercase tracking-[0.18em] sm:tracking-[0.22em] text-[#BF9972] mr-1 font-sans not-italic">
                        {p.label}
                      </span>
                    )}
                    {p.primary}
                  ...
                  {p.compareAt && (
                    <span
                      ...
                      className="hidden sm:inline text-white/40 line-through text-xs flex-shrink-0"
                    >
                      {p.compareAt}
                    </span>
                  )}
        ...
        <div className="pt-2.5 sm:pt-3 mt-auto grid grid-cols-2 gap-1.5 sm:gap-2">
          <Link
            ...
            className="inline-flex items-center justify-center gap-1 bg-[#D4AF37] text-black px-2 sm:px-3 py-2.5 text-[9px] sm:text-[11px] uppercase tracking-[0.12em] sm:tracking-[0.16em] hover:bg-[#B5952F] transition-colors"
          >
            View <ArrowUpRight size={10} className="sm:hidden" /><ArrowUpRight size={12} className="hidden sm:block" />
          </Link>
          <button
            onClick={handleAdd}
            data-testid={`quick-add-${product.id}`}
            className="inline-flex items-center justify-center gap-1 border border-[#D4AF37]/60 text-[#D4AF37] hover:bg-[#D4AF37]/10 px-2 sm:px-3 py-2.5 text-[9px] sm:text-[11px] uppercase tracking-[0.12em] sm:tracking-[0.16em] transition-colors"
          >
            <ShoppingBag size={10} className="sm:hidden" /><ShoppingBag size={12} className="hidden sm:block" /> Inquire
          </button>
        </div>
      </div>
    </div>
