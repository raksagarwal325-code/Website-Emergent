...
beforeEach(() => {
  mockApi.listAllProducts.mockResolvedValue([
    { id: "amber", sku: "SGE-CH-101", name: "Neelpushp Amber Chandelier", category: "Chandelier", status: "published", images: ["/amber.jpg"] },
    { id: "clear", sku: "SGE-CH-102", name: "Neelpushp Clear Chandelier", category: "Chandelier", status: "published", images: ["/clear.jpg"] },
...
test("review shows selected variants first with images and a visible private approval action", async () => {
  ...
  await waitFor(() => expect(mockApi.updateSettings).toHaveBeenCalledWith(expect.objectContaining({
    homepage_content: expect.objectContaining({
      variant_families: [expect.objectContaining({ name: "Neelpushp", product_ids: ["amber", "clear"] })],
    }),
  })));
});
