...
export default function AdminProjectGallery() {
  ...
  const addProject = () => patchGallery("items", [...gallery.items, { ...emptyProject }]);
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-white/40 mt-1">All project and case-study information now lives together in each project card.</p>
          </div>
        ...
        {gallery.items.map((project, index) => (
          <article key={index} className="border border-white/10 p-5 md:p-6 space-y-5 bg-black/10" data-testid={`project-editor-${index}`}>
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-[10px] uppercase tracking-[0.24em] text-[#D4AF37]">Project {index + 1}</div>
                <div className="font-serif text-xl mt-1">{project.title || "Untitled project"}</div>
                {project.location && <div className="text-xs text-white/45 mt-1">{project.location}</div>}
              </div>
              <button type="button" onClick={() => removeProject(index)} className="text-white/40 hover:text-red-400" aria-label="Remove project"><Trash2 size={15} /></button>
            </div>
