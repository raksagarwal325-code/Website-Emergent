...
export default function GalleryPreview() {
  ...
  const headingX = useTransform(scrollYProgress, [0, 1], [0, -90]);
  ...
  return (
    <section ref={sectionRef} data-testid="home-gallery-preview" className="relative h-auto md:h-[240vh] border-t border-[#BF9972]/15">
      <div className="relative md:sticky md:top-0 md:h-screen overflow-hidden flex items-center py-24 md:py-0">
        ...
        <div className="relative max-w-7xl mx-auto px-6 w-full">
          <motion.div className="mb-10 md:mb-12 max-w-4xl" style={{ x: prefersReducedMotion || isMobile ? 0 : headingX }} initial={prefersReducedMotion ? false : "hidden"} whileInView="visible" viewport={{ once: true, amount: 0.35 }} variants={editorialGroup}>
            <motion.div variants={prefersReducedMotion ? undefined : editorialItem}>
              <div className="eyebrow mb-3">{g.eyebrow || "Installations"}</div>
              <h2 className="font-serif text-4xl md:text-6xl lg:text-7xl leading-[1.02]">{g.title_pre || "Our Work"} <span className="brand-gradient-text italic">{g.title_highlight || "in the wild."}</span></h2>
              <div className="mt-5 flex items-center gap-5"><Link to="/gallery" data-testid="home-gallery-view-all" className="inline-flex items-center gap-2 text-sm font-medium uppercase tracking-[0.22em] text-[#D4AF37] hover:text-[#E0C15D] transition-colors">View full gallery <ArrowUpRight size={15} /></Link><span className="hidden md:inline text-[10px] uppercase tracking-[0.28em] text-white/35">Scroll to travel through projects</span></div>
            </motion.div>
          </motion.div>

          <motion.div className="relative px-5 lg:px-7" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} data-testid="home-gallery-carousel" initial={prefersReducedMotion ? false : { opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.18 }} transition={prefersReducedMotion ? { duration: 0 } : { duration: 0.9, ease: LUXURY_EASE }}>
                ...
                {slides.map((slide, sIdx) => <div key={sIdx} className="shrink-0" style={{ width: `${100 / Math.max(1, total)}%` }} aria-hidden={sIdx !== active}>
                  <div className={`grid grid-cols-1 ${gridCols} gap-5 md:gap-8 ${perView === 6 ? "md:grid-rows-2" : perView === 9 ? "md:grid-rows-3" : ""}`}>
                    {slide.map((p, i) => {
                      ...
                      const offsetClass = i % 3 === 1 ? "md:translate-y-10" : i % 3 === 2 ? "md:-translate-y-4" : "";
