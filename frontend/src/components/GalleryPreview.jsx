...
export default function GalleryPreview() {
  ...
  const scrollToIndex = useCallback((index) => {
    ...
    if (rail && card) rail.scrollTo({ left: card.offsetLeft - rail.offsetLeft, behavior: prefersReducedMotion ? "auto" : "smooth" });
    setActive(next);
  ...
  return (
    <section data-testid="home-gallery-preview" className="relative isolate overflow-hidden border-t border-[#BF9972]/15 bg-[#16070f] px-6 py-10 md:py-12">
      ...
      <div className="relative mx-auto max-w-[1500px]">
        <motion.div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between" initial={prefersReducedMotion ? false : "hidden"} whileInView="visible" viewport={{ once: true, amount: 0.35 }} variants={editorialGroup}>
        ...
        <motion.div initial={prefersReducedMotion ? false : { opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.15 }} transition={prefersReducedMotion ? { duration: 0 } : { duration: 0.7, ease: LUXURY_EASE }} className="relative">
          <div ref={railRef} onScroll={onScroll} data-testid="home-gallery-carousel" className="flex snap-x snap-mandatory gap-4 overflow-x-auto overscroll-x-contain pb-2 pr-[14vw] md:gap-5 md:pr-[8vw]" style={{ scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
            {ordered.map((p, index) => {
              const cover = (p.images || []).filter(Boolean)[0];
              return (
                <article key={`${p.__idx}-${p.__slug}`} data-testid={`home-gallery-card-${index}`} className="group w-[76vw] max-w-[350px] shrink-0 snap-start border border-white/8 bg-[#0e0510] transition-all duration-700 hover:border-[#D4AF37]/50 sm:w-[48vw] md:w-[31vw] lg:w-[24vw] xl:w-[22vw]">
                  <Link to={`/gallery/${p.__slug}`} className="block">
                    <div className="aspect-[16/10] overflow-hidden bg-black">{cover ? <img src={api.resolveImage(cover)} alt={p.title || "Project"} loading="lazy" className="h-full w-full scale-[1.01] object-cover opacity-90 transition-all duration-[900ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.04] group-hover:opacity-100" /> : <div className="flex h-full w-full items-center justify-center font-serif italic text-white/25">Image pending</div>}</div>
                    <div className="min-h-[86px] p-4">{p.location && <div className="mb-1.5 inline-flex items-center gap-1.5 text-[9px] uppercase tracking-[0.2em] text-[#BF9972]"><MapPin size={11} strokeWidth={1.5} /> {p.location}</div>}<h3 className="line-clamp-2 font-serif text-base leading-snug text-white transition-colors group-hover:text-[#D4AF37] md:text-lg">{p.title}</h3></div>
                  </Link>
                </article>
              );
            })}
        ...
        {total > 1 && <div className="mt-4"><div className="h-px overflow-hidden bg-white/10"><div className="h-px bg-[#D4AF37] transition-[width] duration-300" style={{ width: `${((active + 1) / total) * 100}%` }} /></div><div className="mt-2 flex items-center justify-end"><div className="text-[9px] uppercase tracking-[0.24em] text-white/35">{String(active + 1).padStart(2, "0")} / {String(total).padStart(2, "0")}</div></div></div>}
      </div>
    </section>
  );
}
