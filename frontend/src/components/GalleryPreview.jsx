    ...
    <section ref={sectionRef} data-testid="home-gallery-preview" className="relative isolate overflow-hidden border-t border-[#BF9972]/15 bg-[#16070f] py-12 md:py-16">
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        <motion.div initial={prefersReducedMotion ? false : { opacity: 0, y: 36, scale: .985 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, amount: .14 }} transition={{ duration: .9, ease: LUXURY_EASE }} drag={prefersReducedMotion ? false : "x"} dragConstraints={{ left: 0, right: 0 }} dragElastic={0.08} onDragEnd={(_, info) => { if (info.offset.x < -80) go(1); if (info.offset.x > 80) go(-1); }} className="relative grid gap-5 md:grid-cols-[.32fr_1fr_.32fr] md:items-center">
          <motion.button type="button" aria-label={`Previous project: ${prev.title || "Previous project"}`} data-testid="home-gallery-side-prev" onClick={() => go(-1)} className="group relative hidden h-[300px] overflow-hidden border border-white/10 bg-black opacity-[.48] transition hover:opacity-80 md:block" whileHover={prefersReducedMotion ? undefined : { x: -6, scale: .985 }} transition={{ duration: .35 }}>
          ...
          <motion.button type="button" aria-label={`Next project: ${next.title || "Next project"}`} data-testid="home-gallery-side-next" onClick={() => go(1)} className="group relative hidden h-[300px] overflow-hidden border border-[#D4AF37]/20 bg-black opacity-[.54] transition hover:opacity-[.86] md:block" whileHover={prefersReducedMotion ? undefined : { x: 6, scale: .985 }} transition={{ duration: .35 }}>
