...
export default function CollageSection() {
  ...
  return (
    <section data-testid="collage-section" className="relative overflow-hidden">
      ...
      <svg aria-hidden="true" viewBox="0 0 200 200" className="absolute -top-10 -right-10 w-[420px] h-[420px] opacity-[0.045] pointer-events-none" fill="none" stroke="#D4AF37" strokeWidth="0.6">
      ...
      <div className="relative max-w-5xl mx-auto px-6 py-24 md:py-32 text-center">
            ...
            return (
              <div key={i} data-testid={`collage-stat-${i}`} className="warm-panel rounded-xl p-6 md:p-7 text-left group hover:border-[#D4AF37]/45 transition-colors relative overflow-hidden">
                ...
                <div className="relative">
                  ...
                  <div className="text-xs md:text-[13px] font-medium uppercase tracking-[0.18em] text-white/70 mt-3">{s.label}</div>
                </div>
              </div>
            );
          })}
        ...
        <div className="mt-12 flex flex-wrap justify-center gap-3">
          <Link to={c.primary_cta_link || "/catalog"} data-testid="collage-explore-btn" className="inline-flex items-center gap-2 bg-[#D4AF37] text-black px-8 py-4 uppercase text-xs tracking-[0.24em] hover:bg-[#B5952F] transition-colors">
            {c.primary_cta_text} <ArrowUpRight size={14} />
          </Link>
          {c.secondary_cta_text && (secondaryExternal ? (
            <a href={secondaryHref} target="_blank" rel="noreferrer" data-testid="collage-wa-btn" className="inline-flex items-center gap-2 border border-[#D4AF37]/60 text-[#D4AF37] hover:bg-[#D4AF37]/10 px-8 py-4 uppercase text-xs tracking-[0.24em] transition-colors">
              <MessageCircle size={14} /> {c.secondary_cta_text}
            </a>
          ) : (
            <Link to={secondaryHref} data-testid="collage-wa-btn" className="inline-flex items-center gap-2 border border-[#D4AF37]/60 text-[#D4AF37] hover:bg-[#D4AF37]/10 px-8 py-4 uppercase text-xs tracking-[0.24em] transition-colors">
              <MessageCircle size={14} /> {c.secondary_cta_text}
            </Link>
          ))}
        </div>
