...
export default function CollageSection() {
  ...
  const ghostScale = useTransform(scrollYProgress, [0, .5, 1], [.98, 1.015, .99]);
  ...
  return (
    <section ref={sectionRef} data-testid="collage-section" className="relative overflow-hidden border-y border-white/10 bg-[#17080f]">
      ...
      <motion.div aria-hidden className="pointer-events-none absolute bottom-5 right-6 font-serif text-[11vw] leading-none text-white/[0.032] md:bottom-4 md:right-8 md:text-[9.5rem]" style={{ y: prefersReducedMotion ? 0 : ghostY, scale: prefersReducedMotion ? 1 : ghostScale }}>1981</motion.div>

      <div className="relative mx-auto max-w-7xl px-6 py-14 md:py-20">
        <motion.div className="grid gap-10 lg:grid-cols-[.95fr_1.55fr] lg:items-center" initial={prefersReducedMotion ? false : "hidden"} whileInView="visible" viewport={{ once: true, amount: 0.18 }} variants={editorialGroup}>
          <motion.div variants={prefersReducedMotion ? undefined : editorialItem} className="max-w-xl">
            ...
            <h2 className="font-serif text-4xl leading-[1.02] sm:text-5xl lg:text-[3.35rem] xl:text-[3.65rem]"><span className="brand-gradient-text">{c.title}</span>{c.highlight && <span className="block text-white">{c.highlight}</span>}</h2>
