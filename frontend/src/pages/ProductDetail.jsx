        ...
        <div className="lg:col-span-5 space-y-8">
          ...
          <div data-testid="buying-confidence" className="border border-white/10 bg-white/[0.02] p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-sm text-white/70">
              <div>✓ Secure Pan-India delivery</div><div>✓ Transit-damage replacement</div><div>✓ Custom sizes & finishes</div><div>✓ Installation guidance</div><div>✓ GST invoice available</div>
            </div>
            <button type="button" data-testid="buying-confidence-shipping-link" onClick={openShippingTab} className="mt-4 text-[10px] uppercase tracking-[0.22em] text-[#BF9972] hover:text-[#D4AF37]">
          ...
          {isMadeToOrder(product) && (
            <p data-testid="made-to-order-note" className="text-xs text-white/60 leading-relaxed max-w-md">
              <span className="text-[#D4AF37]">Pre-order.</span>{" "}Production and dispatch timelines will be confirmed after your enquiry.
            </p>
          )}
          <div className="text-xs text-white/50 border-t border-white/10 pt-6">{product.stock > 0 ? `${product.stock} in stock` : "Available on request"}</div>
        </div>
      </div>

      <ProductTabs product={product} settings={settings} waLink={waLink} active={activeTab} onSelect={setActiveTab} sectionRef={productTabsRef} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
          ...
          <div className="md:col-span-7 space-y-6">
            {reviews.length === 0 && !reviewSubmitted && (
              <div data-testid="reviews-empty-prompt" className="border border-white/10 bg-white/[0.02] p-6 md:p-7">
                <div className="font-serif text-xl md:text-2xl text-white leading-snug">Purchased this piece?</div>
                <p className="mt-1 text-white/60 text-sm md:text-[15px]">Share your experience.</p>
                <button type="button" data-testid="write-review-cta" onClick={() => {
            ...
            {reviews.map((r) => (
              <div key={r.id} data-testid={`review-${r.id}`} className="border border-white/10 p-6">
                <div className="flex items-center justify-between">
                  <div><div className="font-serif text-lg">{r.title || r.author}</div><div className="text-xs text-white/40 mt-1">by {r.author}</div></div>
...
function ProductTabs({ product, settings, waLink, active, onSelect, sectionRef }) {
  ...
  const glanceSpecKeys = ["Height", "Width", "Diameter", "Dimensions", "Material", "Glass", "Crystal", "Finish", "Lights", "Number of Lights", "Holder", "Wattage", "Weight"];
  const glanceSpecs = glanceSpecKeys.filter((key) => isMeaningfulSpec(product?.specs?.[key])).slice(0, 6);
  ...
  return (
    <section ref={sectionRef} className="mt-20 border-t border-white/10 pt-12" data-testid="product-tabs">
      <div className="flex flex-wrap gap-x-8 gap-y-2 border-b border-white/10 mb-10">
        {TABS.map((t) => (
          <button key={t.key} data-testid={`tab-${t.key}`} onClick={() => onSelect(t.key)} className={`relative py-4 text-xs uppercase tracking-[0.28em] transition-colors ${active === t.key ? "text-[#D4AF37]" : "text-white/50 hover:text-white"}`}>
            {t.label}{active === t.key && <span className="absolute left-0 right-0 -bottom-px h-px bg-[#D4AF37]"></span>}
          </button>
        ))}
      </div>
      ...
      <div className="min-h-[240px] fade-up" key={active}>
        {active === "description" && (
          <div data-testid="tab-content-description" className="grid grid-cols-1 md:grid-cols-12 gap-10">
            <div className="md:col-span-8">
              ...
              {product.tags?.length > 0 && <div className="mt-8 flex flex-wrap gap-2">{product.tags.map((t) => <span key={t} className="text-[10px] uppercase tracking-[0.24em] border border-white/15 px-3 py-1 text-white/70">{t}</span>)}</div>}
            </div>
            <div className="md:col-span-4 border border-white/10 p-6">
              ...
              <ul className="space-y-2 text-sm text-white/70">
                ...
                {glanceSpecs.map((key) => <li key={key}>{key} · <span className="text-white">{String(product.specs[key])}</span></li>)}
        ...
        {active === "specifications" && (
          <div data-testid="tab-content-specifications">
            ...
            {specEntries.length > 0 && <div className="border border-white/10 overflow-hidden"><table className="w-full text-sm"><tbody>{specEntries.map(([k, v], i) => <tr key={k} data-testid={`spec-row-${k}`} className={`${i % 2 === 0 ? "bg-white/[0.02]" : "bg-transparent"} border-b border-white/5 last:border-b-0`}><th scope="row" className="text-left align-top py-4 px-6 md:w-1/3 text-white/50 uppercase text-[11px] tracking-[0.22em] font-normal">{k}</th><td className="py-4 px-6 text-white leading-relaxed whitespace-pre-wrap">{String(v)}</td></tr>)}</tbody></table></div>}
            <div data-testid="spec-customization-note" className="mt-6 border-l-2 border-[#D4AF37] bg-white/[0.02] px-5 py-4 text-sm text-white/75 leading-relaxed"><span className="text-[#D4AF37] font-serif italic">Pre-order — </span>Specifications can be customised as per requirement. Please inquire for exact size, holder type, finish, and pricing.</div>
          </div>
        )}

        {active === "shipping" && (
          <div data-testid="tab-content-shipping" className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="border border-white/10 p-6"><div className="flex items-center gap-3 mb-3"><Truck size={16} className="text-[#D4AF37]" /><div className="eyebrow">Shipping</div></div><div className="text-white text-sm mb-1">{settings?.delivery_info || "Pan-India shipping · 7–10 business days"}</div><p className="text-white/60 text-sm leading-relaxed mt-2">All items are carefully packaged in double-corrugated boxes with foam inserts. Larger chandeliers ship in custom crates.</p></div>
            <div className="border border-white/10 p-6"><div className="flex items-center gap-3 mb-3"><CreditCard size={16} className="text-[#D4AF37]" /><div className="eyebrow">Payments</div></div><div className="text-white text-sm mb-1">{settings?.payment_methods || "UPI · Net Banking"}</div><p className="text-white/60 text-sm leading-relaxed mt-2">Confirm your order via WhatsApp — we&apos;ll share UPI ID or bank details. GST invoice provided.</p></div>
            <div className="border border-white/10 p-6"><div className="flex items-center gap-3 mb-3"><MapPin size={16} className="text-[#D4AF37]" /><div className="eyebrow">Origin</div></div><div className="text-white text-sm mb-1">Firozabad, Uttar Pradesh</div><p className="text-white/60 text-sm leading-relaxed mt-2">Ships from our workshop. Transit damage? We replace at our cost — just share an unboxing photo within 48 hours.</p></div>
          </div>
        )}

        {active === "inquiry" && (
          <div data-testid="tab-content-inquiry" className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="border border-white/10 p-8"><div className="eyebrow mb-3">Chat instantly</div><h3 className="font-serif text-2xl mb-3">Talk to us on WhatsApp</h3><p className="text-white/60 text-sm leading-relaxed mb-6">Get availability, custom size quotes, bulk pricing, or installation advice within minutes.</p><a data-testid="tab-wa-btn" href={waLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-[#D4AF37] text-black px-6 py-3 uppercase text-xs tracking-[0.28em] hover:bg-[#B5952F]"><MessageCircle size={14} /> Chat on WhatsApp</a></div>
            <div className="border border-white/10 p-8"><div className="eyebrow mb-3">Email or basket</div><h3 className="font-serif text-2xl mb-3">Send us a detailed inquiry</h3><p className="text-white/60 text-sm leading-relaxed mb-6">Add this piece to your inquiry basket along with others, then submit — we&apos;ll respond by email or phone.</p><div className="flex flex-wrap gap-3"><Link to="/cart" className="inline-flex items-center gap-2 border border-white/25 hover:border-[#D4AF37] px-6 py-3 uppercase text-xs tracking-[0.28em]"><ShoppingBag size={14} /> Open basket</Link><a href={`mailto:${settings?.admin_email || "samratglassemp@gmail.com"}?subject=${encodeURIComponent(`Inquiry: ${product.name} (${product.sku})`)}`} className="inline-flex items-center gap-2 border border-white/25 hover:border-[#D4AF37] px-6 py-3 uppercase text-xs tracking-[0.28em]">Email us</a></div></div>
          </div>
        )}
      </div>
    </section>
