...
export default function CollectionsIndex() {
  ...
  const collections = useMemo(() => {
    ...
    return Array.from(slugs)
      .map((slug) => getCollectionFromProducts(products, slug))
      .filter(Boolean)
      .map((collection) => {
  ...
  return (
    <div data-testid="collections-index" className="max-w-7xl mx-auto px-6 py-16 md:py-20">
      ...
      {loading ? (
        <div className="py-20 text-white/40">Loading collections…</div>
      ) : collections.length === 0 ? (
        <div className="border border-white/10 p-10 text-white/50">
          ...
          {collections.map(({ collection, members, categories, cover }) => {
            ...
            return (
              <Link
                ...
                className="group border border-white/10 bg-white/[0.02] hover:border-[#D4AF37]/50 transition-colors overflow-hidden"
              >
                <div className="aspect-[16/9] bg-black/40 overflow-hidden flex items-center justify-center">
                  {image ? (
                    <img
                      src={image}
                    ...
                    <Layers3 size={36} className="text-white/20" strokeWidth={1} />
                  )}
                </div>
                <div className="p-6 md:p-8">
                  <div className="flex items-start justify-between gap-6">
                    <div>
                      <div className="eyebrow text-[#D4AF37] mb-3">{categories.size} {categories.size === 1 ? "category" : "categories"}</div>
                      <h2 className="font-serif text-3xl md:text-4xl">{collection.name}</h2>
                    </div>
                    <ArrowUpRight size={18} className="text-[#D4AF37] shrink-0 mt-1" />
                  </div>
                  <p className="text-white/50 text-sm leading-relaxed mt-4">{collection.description}</p>
                  <div className="mt-6 text-[10px] uppercase tracking-[0.24em] text-white/35">
